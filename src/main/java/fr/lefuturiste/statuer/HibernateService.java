...
public class HibernateService {
  ...
  public static EntityManager getEntityManager() {
    if (entityManager == null) {
      ...
      App.logger.info("Using connection username " + password.substring(0, 1)
          + password.substring(1).replaceAll("[a-zA-Z0-9]", "*"));
      configuration.setProperty("hibernate.connection.url", connectionUrl);
      configuration.setProperty("hibernate.connection.username", username);
      configuration.setProperty("hibernate.connection.password", password);
      String debugConfig = "Hibernate settings " + configuration.getProperty("connection.url") + " "
  ...
  public static void launchConnexionFailurePreventerUtil() {
    ...
    // System.out.println("launchConnexionFailurePreventerUtil with duration " +
