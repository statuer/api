...
public class DiscordNotifier implements NotifierInterface {
  ...
  public void notify(Incident incident) {
    ...
    if (discordWebhook != null) {
      ...
      if (status.equals("up")) {
        embed.addField("Started at", DateTimeFormatter.ISO_INSTANT.format(incident.getStartedAt()), true)
        ...
        embed.addField("Reason", incident.getReason().getMessage(), false).setColor(Color.decode("#e74c3c"))
